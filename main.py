...
def to_upper_and_normalize_case(char):
    """小文字を大文字に、濁音・半濁音を50音の音に変換する
...
def is_valid_input(user_input, key):
    """Return True if user_input is invalid.

    Args:
        key(str): key of user_input
        user_input(str): input value

    Returns:
        True(bool): if user_input is invalid
    """
...
def get_suitable_word(head, tail):
    ...
    """
    regex = f"^.*{tail}$"
    char_series = vocablary.loc[head, :]
    try:
...
def make_ladder():
    ...
    if times == 1:
        try:
            result.insert(-1, get_suitable_word(start_last_char, end_first_char))
    ...
    while True:
        ...
        try:
            word = get_suitable_word(head, tail)
